...
int max=array[0];
int i=1;
	for(i;i<number_of_elements;i++){	//Comparing each array elements.
...
int sorted_array[number_of_elements];//Sorted Array
int i=0,m=1;
int count[10]={0};//Respresents digits 10 Digits 0-9 || Initializing array elements to 0.
	...
	for(m;m<10;m++){//Changing positions
...
void radix_sort(int array[],int number_of_elements){
	...
	int divide=1;
	for(divide;(get_max/divide)>0;divide*=10){	      //Finds how many digits array has(Kaç haneli bir dizi olduğunu bulmak için)
...
printf("Array After Radix Sorting:  ");
	for(i;i<12;i++){//Sorted array is displayed.
